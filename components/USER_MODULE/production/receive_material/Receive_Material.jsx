...
const Receive_Material = ({ navigation, route }) => {
  const scanner_input_ref = useRef(null);
  const modal_input_ref = useRef(null);
  const isFocused = useIsFocused();

  // Halimbawa: Mula sa route params o global auth context
  ...
  const handle_search = async (val) => {
    ...
    try {
      ...
      if (doc_snap.exists()) {
        ...
        // 1. VALIDATION: Check kung na-receive na ang LPN (may laman ang to_received_by)
        if (data?.to_received_by && data.to_received_by.trim() !== "") {
          Vibration.vibrate([100, 50, 100]);
          Alert.alert(
            "Already Received",
            `LPN: ${clean_id} is already received by ${data.to_received_by}.`,
          );
          return;
        }

        // 2. VALIDATION: Check kung may valid na Transfer Order Reference
        if (data?.to_number_ref && data.to_number_ref.trim() !== "") {
          ...
          Alert.alert(
            "Invalid LPN",
            `LPN ${clean_id} does not have a valid transfer order reference and cannot be received.`,
          );
        }
      } else {
        Vibration.vibrate([100, 50, 100]);
      ...
      console.error("Receive Material Scan Error: ", e);
  ...
  const handle_process_receive = async () => {
    ...
    try {
      ...
      // Group LPNs by their `to_number_ref`
      const items_by_to = received_items.reduce((acc, item) => {
        const to_num = item.to_number_ref;
        if (!acc[to_num]) acc[to_num] = [];
        acc[to_num].push(item);
        return acc;
      }, {});

      // 1. UPDATE TBL_INVENTORY_COUNT & FETCH TO DOCUMENTS
      for (const item of received_items) {
        ...
        batch.update(lpn_ref, {
          to_received_by: receiver_fullname,
          warehouse_code: item.to_warehouse_code || item.warehouse_code,
          sbin_code: item.to_sbin_code || item.sbin_code,
        });
      }

      // 2. UPDATE TBL_TRANSFER_ORDER FOR EACH AFFECTED TO_NUMBER
      for (const to_number of Object.keys(items_by_to)) {
        const to_ref = doc(
          firestore_db,
          "DB1_ERP_SYSTEM",
          "TBL_TRANSFER_ORDER",
        ...
        if (to_snap.exists()) {
          const to_data = to_snap.data();
          const scanned_lpns_in_this_to = items_by_to[to_number];
          const scanned_lpn_ids = scanned_lpns_in_this_to.map((i) => i.lpn_id);

          // Update nested transfer_list and lpn_list
          const updated_transfer_list = (to_data.transfer_list || []).map(
            (t_item) => {
              const current_lpn_list = t_item.lpn_list || [];

              // 1. Update matching LPNs inside this specific mat_code / item
              let updated_lpn_list = current_lpn_list.map((lpn) => {
                if (scanned_lpn_ids.includes(lpn.lpn_id)) {
                  return {
                    ...lpn,
                    to_received_by: receiver_fullname,
                    warehouse_code: lpn.to_warehouse_code || lpn.warehouse_code,
                    sbin_code: lpn.to_sbin_code || lpn.sbin_code,
                  };
                }
                return lpn;
              });

              // 2. Check: DAPAT may laman ang lpn_list AT LAHAT ng LPN nito ay may to_received_by
              const has_lpns = updated_lpn_list.length > 0;
              const all_lpns_received =
                has_lpns &&
                updated_lpn_list.every((lpn) => !!lpn.to_received_by);

              return {
                ...t_item,
                lpn_list: updated_lpn_list,
                // Kapag kumpleto na ang LPNs ng MAT_CODE na ito, tsaka lang magiging "Received"
                transfer_status: all_lpns_received
                  ? "Received"
                  : t_item.transfer_status,
                received_date: all_lpns_received
      ...
      Alert.alert(
        "Success",
        `${received_items.length} LPN(s) successfully processed into inbound staging.`,
      );
      set_received_items([]);
    } catch (error) {
      console.error("Error processing receive material: ", error);
      Alert.alert(
        "Transaction Error",
  ...
  const render_item = ({ item, index }) => (
    <View className="bg-white mx-6 rounded-xl border border-slate-200 mb-2 shadow-sm overflow-hidden">
      ...
      {/* BOTTOM SECTION: DESTINATION & QUANTITY */}
      <View className="px-3.5 py-2.5 flex-row items-center justify-between bg-slate-50/50">
        ...
        <View className="flex-row items-center bg-slate-100 px-2 py-0.5 rounded border border-slate-200/50">
          ...
          >
            {item?.to_warehouse_code || "-"} {item?.to_sbin_code || "-"}
          </Text>
        </View>
  ...
  return (
    <SafeAreaView className="flex-1 bg-white" edges={["top"]}>
      ...
      <View className="px-6 pb-4 flex-row items-center border-b border-slate-100">
        ...
        <SquareArrowDown size={24} color="#059669" />
      </View>
      ...
      <View className="flex-1 bg-slate-50">
        {received_items.length === 0 ? (
          ...
          <View className="flex-1 justify-center items-center px-10">
            ...
            >
              Scan the LPN QR code. System will continuously validate and append
          ...
          <View className="flex-1 pt-4">
            ...
            <View className="absolute bottom-0 inset-x-0 bg-white border-t border-slate-200 px-6 py-4 flex-row items-center space-x-3 gap-3">
              ...
              >
                ...
                >
                  Receive Materials ({received_items.length})
                </Text>
              </TouchableOpacity>
            </View>
          </View>
        ...
        >
          <View className="bg-white rounded-[24px] p-6 shadow-xl">
            ...
            <Text className="text-xs text-slate-500 mb-4">
              Manually type the exact License Plate Number (LPN) for receiving
              validation.
            </Text>
