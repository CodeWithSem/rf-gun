...
// Updated Data Structure to match Activation Check logic
const POSM_DATA = [
  {
    id: "1",
    title: "Brand Header Poster",
    type: "Poster",
    placement: "Main Aisle Wall",
    condition_req: "No tears, No fading",
    status: "priority",
  },
  {
    id: "2",
    ...
    type: "Shelf Strip",
    placement: "Soda Section",
    condition_req: "Aligned & Clean",
    status: "standard",
  },
  {
    id: "3",
    title: "New Flavor Wobblers",
    type: "Wobbler",
    placement: "All SKU Facings",
    condition_req: "Springy & Visible",
    status: "priority",
  },
];

const POSM_Audit = ({ navigation, route }) => {
  ...
  const handleStatus = (id, status) => {
    setAuditData((prev) => ({
      ...prev,
      [id]: { ...prev[id], condition: status },
    }));
  };

  const renderPOSM = ({ item }) => {
    const auditStatus = auditData[item.id]?.condition;
    const isPriority = item.status === "priority";

    return (
      <View className="bg-white p-5 rounded-xl mb-3 border border-slate-200 shadow-sm">
        {/* Header: Material Title & Priority Badge */}
        <View className="flex-row justify-between items-start mb-4">
          <View className="flex-1">
            <View className="flex-row items-center mb-1">
              <ImageIcon size={14} color={isPriority ? "#0284c7" : "#64748b"} />
              <Text
          ...
          {/* Status Badge - Unified with Activation Check design */}
          <View
        ...
        <View className="bg-slate-50 rounded-xl p-4 mb-5 border border-slate-200">
          ...
          <View
            className={`h-[1px] mb-3 ${isPriority ? "bg-sky-100" : "bg-slate-200"}`}
          />
          <View className="flex-1">
        ...
        <View className="flex-row gap-2">
          <TouchableOpacity
            onPress={() => handleStatus(item.id, "Good")}
            className={`flex-1 flex-row items-center justify-center py-3 rounded-xl border ${auditStatus === "Good" ? "bg-emerald-50 border-emerald-500" : "bg-white border-slate-200"}`}
          >
            <CheckCircle2
              size={16}
              color={auditStatus === "Good" ? "#10b981" : "#94a3b8"}
            />
            <Text
              style={{ fontFamily: "Outfit-Bold" }}
              className={`ml-2 mr-3 text-xs ${auditStatus === "Good" ? "text-emerald-700" : "text-slate-500"}`}
            >
              Good
            </Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            onPress={() => handleStatus(item.id, "Poor")}
            className={`flex-1 flex-row items-center justify-center py-3 rounded-xl border ${auditStatus === "Poor" ? "bg-rose-50 border-rose-500" : "bg-white border-slate-200"}`}
          >
            <XCircle
              size={16}
              color={auditStatus === "Poor" ? "#f43f5e" : "#94a3b8"}
            />
            <Text
              style={{ fontFamily: "Outfit-Bold" }}
              className={`ml-2 mr-2 text-xs ${auditStatus === "Poor" ? "text-rose-700" : "text-slate-500"}`}
            >
              Damaged
            </Text>
          </TouchableOpacity>
  ...
  return (
    <SafeAreaView className="flex-1 bg-white" edges={["top"]}>
      ...
      <View className="px-6 py-4 flex-row items-center justify-between border-b border-slate-100">
        <View className="flex-row items-center">
          ...
          <View className="ml-2">
            ...
            >
              {storeData?.description || "Material Inspection"}
            </Text>
          </View>
        </View>
      </View>
      ...
      {/* List */}
      <FlatList
        data={POSM_DATA.filter((p) =>
          p.title.toLowerCase().includes(searchQuery.toLowerCase()),
        )}
        renderItem={renderPOSM}
