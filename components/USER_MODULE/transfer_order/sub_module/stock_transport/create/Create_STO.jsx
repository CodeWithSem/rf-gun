...
import { get_iso_date_now } from "../../../../../../assets/scripts/format/date_format";

const Create_STO = ({ navigation, route }) => {
  ...
  // Focus Tracker para hindi agawin ng scanner ang cursor kapag nagta-type sa Header
  ...
  // CONTINUOUS AUTO-FOCUS PARA SA BACKGROUND SCANNER (PAUSED KAPAG NAGTA-TYPE SA HEADER)
  useEffect(() => {
  ...
  // AUTO-FOCUS SA HIDDEN SCANNER O MANUAL INPUT SA LOOB NG MODAL
  useEffect(() => {
    let focus_interval = null;
  ...
  const handleSaveHeader = async () => {
    try {
      ...
      set_loading(false);
      Alert.alert("Success", "STO details has been saved.", [
  ...
  const handleConfirmAddLPN = async () => {
    ...
    try {
      ...
      const tagged_lpn_item = {
        ...
        sto_picked_date: get_iso_date_now(),
  ...
  return (
    <SafeAreaView className="flex-1 bg-white relative" edges={["top"]}>
      ...
      <ScrollView
        className="flex-1 px-5 pt-4 sky-50/40"
      ...
      >
        ...
        <TouchableOpacity
          ...
          className="bg-sky-50 border border-dashed border-sky-400 py-3 rounded-xl flex-row items-center justify-center active:bg-sky-100 mb-6"
        >
      ...
      {/* STICKY FOOTER - SAVE INFO BUTTON */}
      <SafeAreaView
        edges={["bottom"]}
